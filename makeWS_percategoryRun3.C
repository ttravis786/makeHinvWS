...
int makeWS_percategoryRun3(
    ...
    double xmin, xmax;
    if (!classifier) {
        varName = "mjj_" + cat + "_" + year; varTitle = "M_{jj} (GeV)"; xmin = 200; xmax = 5000;
    } else {
        varName = "bdt_" + cat + "_" + year; varTitle = "Signal Score"; xmin = 0; xmax = 1;
    }
    ...
    for (unsigned iB = 1; iB <= nB; ++iB) {
        ...
        for (unsigned iT = 0; iT < nT; ++iT) {
            ...
            lname.str(""); lname << lCategory << lYear << lType[iT] << "Z_SR_bin" << iB;
            ...
            if (iT == 0) {
                ...
                lname.str(""); lname << lCategory << lYear << "ewkqcdratio_stat_bin" << iB;
                ewkqcdratiostat[iB] = new RooRealVar(lname.str().c_str(), "EWK/QCD ratio stat", 0);

                lname.str(""); lname << lCategory << lYear << "TF_EWKQCDSR_bin" << iB;
                ...
                lname.str(""); lname << lCategory << lYear << "EWKQCD_SR_bin" << iB;
            ...
            lname.str(""); lname << lCategory << lYear << lType[iT] << "wzratio_stat_bin" << iB;
            ...
            lname.str(""); lname << lCategory << lYear << lType[iT] << "TF_WZSR_bin" << iB;
            ...
            lname.str(""); lname << lCategory << lYear << lType[iT] << "WZ_SR_bin" << iB;
            ...
            for (unsigned iR = 1; iR < nR; ++iR) {
                ...
                double crNom = std::max(0.0, getBin(histos[iR][crProc][0], iB));
                double srNom = std::max(0.0, getBin(histos[0][srProc][0], iB));
                ...
                lname.str(""); lname << lCategory << lYear
                                     << lType[iT] << "TF_" << lRegionAlias[iR] << "_stat_bin" << iB;
                TFstat[iT][iB][iR-1] = new RooRealVar(lname.str().c_str(), "CR/SR stat", 0);

                lname.str(""); lname << lCategory << lYear
                                     << lType[iT] << "TF_" << lRegionAlias[iR] << "_bin" << iB;
                ...
                for (unsigned iN = 0; iN < nN; ++iN) {
                    ...
                    double rsUp = hardCodeNuisance[iR][iSup] >= 0
                        ...
                        : 1. + (safeDiv(getBin(hCRup, iB, crNom), getBin(hSRup, iB, srNom)) - tfRatio) / tfRatio;
                    double rsDn = hardCodeNuisance[iR][iSdn] >= 0
                        ? hardCodeNuisance[iR][iSdn]
                        : 1. + (safeDiv(getBin(hCRdn, iB, crNom), getBin(hSRdn, iB, srNom)) - tfRatio) / tfRatio;
                ...
                lname.str(""); lname << lCategory << lYear
                                     << lType[iT] << "V_" << lRegionAlias[iR] << "_bin" << iB;
    ...
    for (unsigned iT = 0; iT < nT; ++iT) {
        for (unsigned iB = 0; iB < nB; ++iB) {
            ...
            // Z SR bins
            if (iT == 0) {
                lname.str(""); lname << lCategory << lYear << lType[iT] << "Z_SR_bin" << iB+1;
                ...
                lname.str(""); lname << lCategory << lYear << "EWKQCD_SR_bin" << iB+1;
            ...
            // W SR bins
            lname.str(""); lname << lCategory << lYear << lType[iT] << "WZ_SR_bin" << iB+1;
            if (!wspace.function(lname.str().c_str())) {
                std::cout << "ERROR: missing " << lname.str() << std::endl; return 1;
            }
            ...
            for (unsigned iR = 1; iR < nR; ++iR) {
                lname.str(""); lname << lCategory << lYear
                                     << lType[iT] << "V_" << lRegionAlias[iR] << "_bin" << iB+1;
                if (!wspace.function(lname.str().c_str())) {
                    std::cout << "ERROR: missing " << lname.str() << std::endl; return 1;
                }
    ...
    for (unsigned iT = 0; iT < nT; ++iT) {
        ...
        for (unsigned iR = 1; iR < nR; ++iR) {
            std::ostringstream lname;
            lname << lCategory << lYear << lType[iT] << "V_" << lRegionAlias[iR];
